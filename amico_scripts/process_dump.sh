...
#python vt_cron.py >& $PWD/logs/vt_cron.log &
	...
	for i in $dumped_files; do
        ...
        if [ -z "$host" ]; then
            ...
            if [ -n "$sha1" ]; then	
                ...
                if [ -z "$corrupt" ]; then
                    python db_virus_total.py $sha1 $md5 $dump_id
                fi

                python manual_download.py $sha1 &
                #python virus_total_submissions.py $sha1 &
                python ip2asn.py $sha1
                
                if [ -z "$corrupt" ]; then
                     python get_feature_vector.py $dump_id
                     python classify_one.py $dump_id
                     python db_syslog.py $sha1 $dump_id &
                fi
